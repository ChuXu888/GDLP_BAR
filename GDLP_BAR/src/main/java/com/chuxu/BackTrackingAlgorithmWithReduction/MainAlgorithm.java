...
import com.chuxu.BackTrackingAlgorithmWithReduction.part01_preHandleData.SmallCase.SmallCase;
...
//        for (double[] matrix : disMatrix) System.out.println(Arrays.toString(matrix));
        ...
        //Step5：调用降阶子算法，对问题进行降阶；
        OrderReductionAlgorithm.orderReductionAlgorithm();

        //Step6：调用回溯子算法Backtrack(1)，获取问题的最优解S*=S_best。
        initialList();
...
//        System.out.println("countOfConflictList中的元素个数 = " + ConflictList.size());
    ...
    public static void initialList() {
        V_list.addAll(V);
        //由于Facility的Comparable接口已经实现过一次用于按照性价比排序了，所以此处要将V_list按照编号排序，只能另写一个排序方法了
//        quickSort(V_list, 0, V_list.size() - 1);
    ...
    public static void deepCopy() {
        disMatrix = SmallCase.disMatrix;
        Pr = P = SmallCase.P;
        Cr = C = SmallCase.C;
        LinkedHashSet<Facility> facilities = SmallCase.V;
